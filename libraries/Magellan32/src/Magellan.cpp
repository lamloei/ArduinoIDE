...
void Magellan::setup_module()
{
  ...
  */
  ...
  delay(6000);
  Serial.print(F("."));
  Serial1.println(F("AT+NCONFIG=AUTOCONNECT,true"));
  delay(6000);
  Serial.println(F("OK"));
  ...
  */
  Serial.println(F("OK"));
  Serial1.println(F("AT+NSOCR=DGRAM,17,5684,1"));
  delay(6000);
  _serial_flush();
  data_input="";
  Serial.print(F(">>IMSI "));
  Serial1.println(F("AT+CIMI"));
  while(1){
...
void Magellan::Msgsend(String payload,String option1,String option2,String option3)
{
  ...
  if(en_post){
      ...
      Serial1.print(F("AT+NSOST=0,"));
      Serial1.print(serverIP);
      Serial1.print(F(",5683,"));
      ...
      Serial1.print(buff_len);
      if(debug) Serial.print(F(",4202"));

      Serial1.print(F(",4202"));
      printmsgID(Msg_ID);
      printmsgID(post_token);                     //print token
      Serial1.print(F("b54e42496f54"));
      ...
      //Serial1.print(F("1132"));                 //content-type json
      Serial1.print(F("ff")); 
...
void Magellan::Msgget(String Resource,String Proxy)
{
  ...
  Serial1.print(buff_len+2);
  if(debug) Serial.print(F(",4201"));
  Serial1.print(F(",4201"));
  printmsgID(Msg_ID);
  printmsgID(get_token);                     //print token
  Serial1.print(F("b54e42496f54"));
  if(debug) Serial.print(F("b54e42496f54"));
  Serial1.print(F("0d17"));
  ...
  Serial1.print(F("8104"));      //Block size 256 recommended for BC95 with CoAP Header
  if(debug) Serial.print(F("8104"));

  //Serial1.print(F("8105"));      //Block size 512 recommended for BC95
  //if(debug) Serial.print(F("8105"));
  //Serial1.print(F("8106"));    //Block size 1024
...
{ 
  ...
  if(en_chk_buff && (current-previous>=500) && !(Serial1.available()))
  {
      Serial1.println(F("AT+NSORF=0,512"));
      cnt_cmdgetrsp++;
      previous=current;
  }
